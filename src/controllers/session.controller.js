...
const upadate = 'UPDATE test_session SET category_id = $1, description = $2,  pre_test_msg = $3, time_limit = $4, updated_at = $5, status = $6 WHERE id = $7'
...
exports.update = (req, res) => {
  const id = req.params.id
  const { category_id, description,  pre_test_msg, time_limit, status} = req.body
  const date = new Date()

  db.query(upadate, [category_id, description,  pre_test_msg, time_limit, date, status, id], (error, results) => {
    if (error) {
      response.error500(res, error.message)
    } else {
